...
function reduce_selection(target_amount, denomination_array, result_array, _coin, _keep)
{
	...
	###################################################################
	# Filthy Hack							  #
	# For some reason (probably a version mismatch)		  #
	# the solution that works locally doesn't work on exercism	  #
	# Add this hack to try solve it				  #
	###################################################################
	if (length(denomination_array) <= 2) {
